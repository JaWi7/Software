...
This script solves the mutual induction to visualize the Bx,By-components of the
inducing+induced magnetic fields in the perfectly conducting case.
...
B_tot_sp = np.sqrt((dipB_oc[0]+dipB_res[0]+B_ind[0])**2 + (dipB_oc[1]+dipB_res[1]+B_ind[1])**2)
B_tot[np.isnan(B_tot)] = 0
U = (dipB_res[0]+dipB_oc[0]+B_ind[0])/B_tot_sp
V = (dipB_oc[1]+dipB_res[1]+B_ind[1])/B_tot_sp
U_ = (fullB_oc[0]+fullB_res[0]+B_ind[0])/B_tot
V_ = (fullB_oc[1]+fullB_res[1]+B_ind[1])/B_tot
...
#CS_r = ax[0].streamplot(cart_grid.X/1e3, cart_grid.Y/1e3, dipB_res[0]+dipB_oc[0], dipB_oc[1]+dipB_res[1]+B_ind[1],density=[0.75], color ='k')
#CS = ax[1].streamplot(cart_grid.X/1e3, cart_grid.Y/1e3, fullB_oc[0]+fullB_res[0], fullB_res[1]+fullB_oc[1]+B_ind[1],density=[0.75], color ='k')
q = ax[0].quiver(cart_grid.X[::step,::step]/1e3, cart_grid.Y[::step,::step]/1e3, U[::step,::step], V[::step,::step],color ='k',units='xy')
q_ = ax[1].quiver(cart_grid.X[::step,::step]/1e3, cart_grid.Y[::step,::step]/1e3, U_[::step,::step], V_[::step,::step],color ='k',units='xy')
matrix_sp =  ax[0].imshow(B_tot_sp, norm=norm, cmap = 'autumn', extent = extent,origin = 'lower')
matrix = ax[1].imshow(B_tot, norm=norm, cmap = 'autumn', extent = extent ,origin = 'lower')
...
for i in range(2):
    ...
    ax[i].plot(p.r_c/1e3 + p.r_res/1e3 * np.cos(np.linspace(0,2*np.pi)), p.r_res/1e3 * np.sin(np.linspace(0,2*np.pi)), color = 'k')
    ax[i].plot(p.r_m/1e3 * np.cos(np.linspace(0,2*np .pi, num=360)), p.r_m/1e3 * np.sin(np.linspace(0,2*np.pi, num=360)), color = 'k', linestyle = 'dashed')
    ax[i].set_xlabel(r"$x$ /km", fontsize=16)
    ax[i].tick_params('both', labelsize=15)
    ax[i].set_ylim(-60,60)
    ax[i].set_xlim(1480,1600)
    ax[i].text(1540, 7,'Res', fontsize=15)
    ax[i].text(1515,-29, 'Icy Crust', fontsize = 15)
    ax[i].text(1481, -12,'Ocean',fontsize=15)
    ax[i].arrow(1540, 0, bind_sc[0]/12, bind_sc[1]/12, width = 0.5,head_width=2, color = 'k', label = 'B_{ind}')
    ax[i].arrow(1540, 0, (g_res[1,1,1]+g_res[0,1,1])/6, (h_res[1,1,1]+h_res[0,1,1])/6, width = 0.5,head_width=2, color = 'cyan')
    ax[i].arrow(1540,0, g_oc[0,1,1]/6, h_oc[0,1,1]/6, width = 0.5,head_width=2, color = 'g')
...
plt.savefig('BField_Arrows.pdf', bbox_inches='tight')
